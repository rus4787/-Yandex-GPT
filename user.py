...
#     #     # Формирование промта для передачи текстов на обработку агентам
...
#         if 'result' not in response_data or 'alternatives' not in response_data['result'] or not response_data['result']['alternatives']:
#             logging.error(f"Ошибка в структуре ответа: {response_data}")
#             return None
#
#         response_text = response_data['result']['alternatives'][0].get('message', {}).get('text', "")
...
#     #     # Прогонка каждого текста через агентную систему
#     #     processor = Processor()
...
#         processed_df['success'] = [res['success'] for res in processed_results]
#         processed_df['processed_text'] = [res['processed_text'] for res in processed_results]
#         processed_df['char_count'] = [res['char_count'] for res in processed_results]
#
...
#     #     processed_df.to_excel(self.output_filepath, index=False)
#
#     def save_processed_data(self):
...
#
#     def run(self):
#         # Основная функция запуска процесса обработки
...
#
# # Новый блок с ответами оценщиков
# import pandas as pd
# import requests
# import json
# import logging
...
#             if processed_text:
#                 result = processor.process(text=processed_text)
#                 модуль_вовлеченности = result.get("Оценщик_1", "") or result.get("Оценщик_2", "") or result.get("Оценщик_3", "")
#                 processed_results.append({
#                     "success": result.get("success", False),
#                     "processed_text": processed_text,
#                     "char_count": result.get("char_count", 0),
#                     "тип_разговора": result.get("лейблы", []),
#                     "модуль_вовлеченности": модуль_вовлеченности,
#                     "не_результат": result.get("Оценщик_4", ""),
#                     "результат": result.get("Оценщик_5", "") + result.get("Оценщик_6", ""),
#                     "другое": result.get("Оценщик_7", ""),
#                     "надо_сказать": ""
#                 })
#             else:
#                 logging.warning(f"Пустой обработанный текст для строки {index}, пропуск.")
...
#
#         # Запрос данных от Буфера для столбца "надо_сказать"
#         logging.info("Передача данных в Буфер для генерации фразы 'надо_сказать'")
#         buffer_response = self.request_buffer_data()
#         if buffer_response is not None and not buffer_response.empty:
#             processed_df['надо_сказать'] = buffer_response['надо_сказать']
#         else:
#             logging.warning("Буфер вернул пустой ответ, столбец 'надо_сказать' не был обновлен.")
#
#         self.processed_data = processed_df
#
